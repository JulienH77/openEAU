...
SESSION.headers.update({
    "User-Agent": "OpenEAU-GitHubActions/2.0",
    "Accept": "application/json",
})

AXES = {
    "marne","seine","yonne","aube","aisne","oise","loire","allier",
    "vienne","cher","sarthe","mayenne","loir","rhone","saone","doubs",
    "isere","ain","drome","garonne","dordogne","lot","tarn","adour",
    "charente","meuse","moselle","rhin","somme","escaut","aude","herault",
}
EXCLUDED = ("ru ","ru de ","ru du ","ru des ","ruisseau","fossé","fosse","ravin","drain","torrent","canal","source","bief")
PRIORITY_1 = {"marne","seine","loire","rhone","saone","garonne","dordogne","meuse","moselle","rhin"}
PRIORITY_2 = {"yonne","aube","aisne","oise","allier","vienne","cher","sarthe","mayenne","lot","tarn","adour","charente","doubs","isere","ain","somme","escaut"}
...
def is_large_river(name: Any) -> bool:
    text = river_key(name)
    if not text or any(x in text for x in EXCLUDED):
        return False
    return any(
        text == axis or text.startswith(axis + " ") or text.startswith(axis + "-") or text.startswith(axis + ",")
        for axis in AXES
    )
...
def is_metropolitan(station: dict[str, Any]) -> bool:
    try:
        lat = float(station.get("latitude_station"))
        lon = float(station.get("longitude_station"))
...
def select_stations(raw: list[dict[str, Any]]) -> list[dict[str, Any]]:
    candidates = [
        s for s in raw
        if s.get("en_service") != 0 and is_metropolitan(s) and is_large_river(s.get("libelle_cours_eau"))
    ]
    candidates.sort(key=lambda s: (importance(s), river_key(s.get("libelle_cours_eau")), str(s.get("code_station", ""))))
    occupied: set[tuple[str, int, int]] = set()
    selected: list[dict[str, Any]] = []
    for s in candidates:
        lat = float(s["latitude_station"])
        lon = float(s["longitude_station"])
        river = river_key(s.get("libelle_cours_eau"))
        key = (river, math.floor(lat / 0.07), math.floor(lon / 0.10))
        if key in occupied:
            continue
        occupied.add(key)
        selected.append(s)
    selected.sort(key=lambda s: (importance(s), river_key(s.get("libelle_cours_eau")), str(s.get("code_station", ""))))
...
def fetch_current(station: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    h = get_json(f"{API}/observations_tr", {"format":"json","code_entite":code,"grandeur_hydro":"H","size":1,"sort":"desc"}).get("data") or []
    q = get_json(f"{API}/observations_tr", {"format":"json","code_entite":code,"grandeur_hydro":"Q","size":1,"sort":"desc"}).get("data") or []
    h0, q0 = (h[0] if h else None), (q[0] if q else None)
    ...
    return code, {"height_m":hm, "flow_m3s":qm, "observed_at":(h0 or q0 or {}).get("date_obs")}
...
def fetch_normal(station: dict[str, Any], month: int) -> dict[str, Any] | None:
    ...
    payload = get_json(f"{API}/obs_elab", {"format":"json","code_entite":site,"grandeur_hydro_elab":"QmM","date_debut_obs_elab":"1950-01-01","date_fin_obs_elab":end,"size":20000})
    vals = []
    for row in payload.get("data") or []:
        d, v = row.get("date_obs_elab"), row.get("resultat_obs_elab")
    ...
    if not vals:
        return None
    return {"normal_m3s":median(vals),"sample_size":len(vals),"month":month}
...
def classify(flow: float | None, normal: float | None) -> tuple[str, float | None]:
    if flow is None or normal is None or normal <= 0:
        return "unknown", None
    ratio = flow / normal
    if ratio < .20: return "very-low", ratio
    if ratio < .50: return "low", ratio
    if ratio <= 1.50: return "normal", ratio
    if ratio <= 2.00: return "high", ratio
...
def fetch_height_history(code: str) -> list[dict[str, Any]]:
    ...
    payload = get_json(f"{API}/observations_tr", {"format":"json","code_entite":code,"grandeur_hydro":"H","date_debut_obs":start.isoformat(),"date_fin_obs":end.isoformat(),"sort":"asc","size":20000})
    result = []
    for row in payload.get("data") or []:
        v = parse_float(row.get("resultat_obs"))
        if v is not None and row.get("date_obs"):
            result.append({"date":row["date_obs"],"value":v/1000})
    return result
...
def fetch_flow_history(site_code: str) -> list[dict[str, Any]]:
    ...
    payload = get_json(f"{API}/obs_elab", {"format":"json","code_entite":site_code,"grandeur_hydro_elab":"QmnJ","date_debut_obs_elab":start.isoformat(),"date_fin_obs_elab":end.isoformat(),"sort":"asc","size":20000})
    result = []
    for row in payload.get("data") or []:
        v = parse_float(row.get("resultat_obs_elab"))
        if v is not None and row.get("date_obs_elab"):
            result.append({"date":row["date_obs_elab"],"value":v/1000})
    return result
...
def write_history(station: dict[str, Any]) -> None:
    ...
    try:
        payload = {
            ...
            "flow": fetch_flow_history(station.get("code_site") or code),
        }
        (HISTORY / f"{code}.json").write_text(json.dumps(payload, ensure_ascii=False, separators=(",",":")), encoding="utf-8")
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[WARN] mesures {s['code_station']}: {exc}")

    normal_jobs = [s for s in selected if normals.get(s["code_station"],{}).get("month") != month]
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if result: normals[code] = result
            except Exception as exc:
                print(f"[WARN] normale {code}: {exc}")
    NORMALS_FILE.write_text(json.dumps(normals, ensure_ascii=False, indent=2), encoding="utf-8")

    station_lookup = {s["code"]:s for s in station_rows}
    live_rows=[]
    for s in selected:
        code=s["code_station"]
        obs=live.get(code,{"height_m":None,"flow_m3s":None,"observed_at":None})
        normal=normals.get(code)
        level,ratio=classify(obs.get("flow_m3s"), normal.get("normal_m3s") if normal else None)
        zero=station_lookup.get(code,{}).get("zero_scale_m")
        water_level = zero + obs["height_m"] if zero is not None and obs.get("height_m") is not None else None
        live_rows.append({
            "code":code,
    ...
    LIVE_FILE.write_text(json.dumps({"updated":datetime.now(timezone.utc).isoformat(),"count":len(live_rows),"stations":live_rows}, ensure_ascii=False, separators=(",",":")), encoding="utf-8")
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for i,future in enumerate(as_completed(futures),1):
            try: future.result()
            except Exception as exc: print(f"[WARN] historique: {exc}")
            if i % 100 == 0: print(f"Historiques : {i}/{len(selected)}")
